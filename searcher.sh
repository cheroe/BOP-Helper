...
# echo "You chose the year $Year the Month of $Month the Broodstock type $Broodstock"

csvcut -c collection_year,collection_month,broodstock,collection_date,shell_height_mm All-ORS-Data-All-Time.csv | csvgrep -c collection_year -r "$Year" | csvgrep -c collection_month -r "$Month" | csvgrep -c broodstock -r "$Broodstock" | csvsort -c collection_date -r | csvlook
